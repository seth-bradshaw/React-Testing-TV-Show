...
test('re-renders with list of episodes', () => {
    //arrange(render component and initialize mock data)
    const { rerender, getAllByTestId, queryAllByTestId } = render(<Episodes episodes={[]}/>)
    //assert there are no episodes yet
    const episodeList = queryAllByTestId(/mission/i)
    expect(episodeList).toHaveLength(0)

    //act(rerender with data passed in)
    rerender(<Episodes episodes={episodesData} />)
    const episodes = getAllByTestId(/episode/i) //get all functions returns an array if it finds multiple elements
    //assert
